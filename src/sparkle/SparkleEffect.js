...
const fragShader = /* glsl */ `
    ...
    void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
        ...
        float distFactor = exp(-dist * dist * 0.005);
        ...
        vec2 offset = normalize(worldPos).xz * 1000.;

        float noise = nn(offset);
        noise = max(nn(offset + 0.01), noise);
        noise = max(nn(offset - 0.01), noise);
        // noise = max(n4rand_ss(normalize(worldPos).yz * 1000. + mat.normal.yz * 500. + 3.01), noise);
        // noise = max(n4rand_ss(normalize(worldPos).xy * 1000. + mat.normal.xy * 500. - 10.1), noise);
        ...
        float sparkleFactor = noise * lum * facing * glossiness * distFactor * 50000. * intensity;

        vec3 color = inputColor.rgb + mat.diffuse.rgb * sparkleFactor;
